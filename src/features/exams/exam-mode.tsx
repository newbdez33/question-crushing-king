...
export function ExamMode({ examId, count, seed, initialQuestionIndex }: ExamModeProps) {
  ...
  const fallbackQuestions: PracticeQuestion[] | null = exam
    ...
    : null
  const [allQuestions, setAllQuestions] = useState<PracticeQuestion[] | null>(fallbackQuestions)
    ...
    async function load() {
      ...
      try {
        ...
        const mappedAll = (data.questions ?? [])
          ...
          .map((q) => {
            const options = (q.options ?? []).slice().sort((a, b) => a.label.localeCompare(b.label))
        ...
        const available = mappedAll.length > 0 ? mappedAll : (fallbackQuestions ?? [])
        let selected = available
        if (typeof count === 'number' && count > 0 && count < available.length) {
          const s = (seed ?? `${Date.now()}`).split('').reduce((acc, ch) => acc + ch.charCodeAt(0), 0)
          const rnd = mulberry32(s)
          const indices = new Set<number>()
          while (indices.size < count) {
            indices.add(Math.floor(rnd() * available.length))
          }
          selected = Array.from(indices).map((i) => available[i]).sort((a, b) => {
  ...
  const question = useMemo(() => (questions ? questions[currentQuestionIndex] : undefined), [questions, currentQuestionIndex])
  ...
  const toggleBookmark = () => {
    ...
    if (userUidRef.current) {
      void RemoteProgress.toggleBookmark(userUidRef.current, examId, question.id, newState)
    } else {
  ...
  const handleSubmit = () => {
    ...
    ProgressService.saveAnswer(userId, examId, question.id, status, selectedAnswers, isCorrect)
    setExamProgress(ProgressService.getExamProgress(userId, examId))
    if (userUidRef.current) {
      const prev = examProgress[question.id]
      void RemoteProgress.saveAnswer(userUidRef.current, examId, question.id, status, selectedAnswers, prev, isCorrect)
    }
                ...
                {question?.type === 'single' ? (
                  <div className='space-y-2 sm:space-y-4'>
                    {question.options.map((option, idx) => {
                      ...
                      return (
                        <div key={idx} className='flex items-center gap-2 sm:gap-3 rounded-md border p-2 sm:p-4'>
                          <input
                            type='radio'
                            name='options'
                            id={`option-${idx}`}
                            checked={isSelected}
                            onChange={() => !isSubmitted && handleSelectAnswer(idx)}
                          ...
                          <Label htmlFor={`option-${idx}`} className='flex-1 cursor-pointer font-normal leading-relaxed text-xs'>
